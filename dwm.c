...
drawstatusbar(Monitor *m, int bh, char* stext) {
	...
	while (text[++i]) {
		if (text[i] == '^' && !isCode) {
			...
			text[i] = '\0';
			w = TEXTW(text) - lrpad;
			drw_text(drw, x, barspacing / 2, w, bh - barspacing, 0, text, 0);
			...
			while (text[++i] != '^') {
				if (text[i] == 'c') {
					...
					drw_rect(drw, rx + x, ry + barspacing / 2, rw, rh, 1, 0);
				} else if (text[i] == 'f') {
					x += atoi(text + ++i);
				}
			}
...
//				urg & 1 << i);
	...
	//drw_map(drw, m->barwin, 0, 0, m->ww, bh); // this line is from old_dwm
...
tile(Monitor *m)
{
	...
	for (i = 0, my = ty = m->gappx, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			...
			if (my + HEIGHT(c) + m->gappx + vp + bh < m->wh)
				my += HEIGHT(c) + m->gappx;
		} else {
			h = (m->wh - ty) / (n - i) - m->gappx;
			resize(c, m->wx + mw + m->gappx, m->wy + ty, m->ww - mw - (2*c->bw) - 2*m->gappx, h - (2*c->bw), 0);
			if (ty + HEIGHT(c) + m->gappx + vp + bh < m->wh)
				ty += HEIGHT(c) + m->gappx;
		}
}
...
updatesystrayicongeom(Client *i, int w, int h)
{
        if (i) {
                ...
                if (w == h)
                        ...
                        i->w = (int) ((float)lrpad * ((float)w / (float)h));
                ...
                if (i->h > lrpad) {
                        if (i->w == i->h)
                                ...
                                i->w = (int) ((float)lrpad * ((float)i->w / (float)i->h));
                        i->h = lrpad;
                }
                if (i->w > 2*lrpad)
                        i->w = lrpad;
...
updatesystray(void)
{
	...
	for (w = 0, i = systray->icons; i; i = i->next) {
		...
		w += systrayspacing;
		i->x = w;
		XMoveResizeWindow(dpy, i->win, i->x, barspacing / 2, i->w, i->h);
		w += i->w;
		if (i->mon != m)
			i->mon = m;
	}
...
centeredmaster(Monitor *m)
{
        ...
        if (n == 0)
                return;
        /* Giang's modification: go fullscreen if only one client */
        if(n == 1){
        ...
        if (i < m->nmaster) {
                ...
                if(my + HEIGHT(c) + m->gappx + vp + bh < m->mh)
                        my += HEIGHT(c) + m->gappx;
        } else {
